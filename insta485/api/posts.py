...
#   # Get query strings used to filter
#   limit = request.args.get('size', 10, type=int) # Default is limit of 10
#   postid_lte = request.args.get('postid_lte', session.get('last_post', None), type=int)
#   if postid_lte and session.get('last_post'):
#     postid_lte = min(request.args.get('postid_lte'), int(session['last_post']))
#   page = request.args.get('page', 1, type=int) 
#   print(postid_lte)
#   print()
#   print()
...
#       "ORDER BY posts.postid DESC "
#       "LIMIT ? OFFSET ?",
#       (username, username, username, limit, (limit * (page - 1)))
#     ).fetchall()
#   else:
#     posts = connection.execute(
#       "SELECT DISTINCT posts.postid, posts.owner, posts.created, "
...
#       "ORDER BY posts.postid DESC "
#       "LIMIT ? OFFSET ?",
#       (username, username, username, postid_lte, limit, (limit * (page - 1)))
#     ).fetchall()
...
#     next = ""
#   else:
#     next = f"/api/v1/posts/?size={limit}&page={page}&postid_lte={postid_lte}"
